...
const App = () => {
  ...
  const getShows = async() => {
    try {
      ...
      await fetch("https://proxy.cors.sh/https://api.themoviedb.org/3/discover/movie?api_key=1abb3e68d878be1155d781ce812f80a8",{
        headers: {
  ...
  <div className="movies-container">
    {shows.map((data) => (
      <div className='moviecards' key={data.id}>
        ...
        <p className='titles'>Title: <span>{data?.title}</span></p>
        <a href={src + data.id}>
